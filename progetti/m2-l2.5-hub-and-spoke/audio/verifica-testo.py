...
RESE = [
 # PV 2.1: «uno virgola sei» nel copione, «1,6» nella trascrizione (la virgola
 # e' gia' tolta qui): si toglie la parola «virgola» fra due numeri, e le due
 # forme ricadono nella stessa regola dei rimandi qui sotto.
 (r"\b(uno|due|tre|quattro|cinque|sei|sette|otto|nove|zero)\s+virgola\s+", r" \1 "),
 # PV modulo 2: i rimandi alle lezioni «due punto quattro», in cifre «2.4».
 *[(rf"\b(due|2)[\s.]+(punto[\s.]+)?({n}|{c})\b", f" lezione2{c} ")
   for n, c in (("uno","1"),("due","2"),("tre","3"),("quattro","4"),("cinque","5"),("sei","6"),("sette","7"))],
 ...
 # «a pie' di lista» (PV 1.1): il trascrittore scrive «a piedi lista», che
 # suona identico. Resa dichiarata, non tolleranza.
 (r"\bpiedi\s+lista\b",                " pie di lista "),
 # PV 1.3-1.5: la sigla DRG si scrive come si pronuncia («di erre gi») e il
 # trascrittore la riporta incollata. Stessa resa della sigla sillabata.
 (r"\bdi\s+erre\s+gi\b",              " drg "),
 (r"\bd\s+r\s+g\b",                    " drg "),
 # PV 1.5-1.6: «Titolo quinto» nel copione, «Titolo V» o «quinto» nella trascrizione.
 (r"\btitolo\s+(v|quinto)\b",           " titolo quinto "),
 (r"\bsocio\s+sanitari([aeio])\b",      r" sociosanitari\1 "),
]
