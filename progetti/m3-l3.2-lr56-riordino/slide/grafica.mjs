...
// Come num, ma per il testo a schermo: in italiano la virgola decimale.
const virg = n => num(n).replace('.', ',');
...
/* PV 2.4: nove righe (le ULSS venete) sforano anche in «fitta»: da 8 righe in su si stringe ancora. */
.tab.fittissima td{font-size:27px;padding:12px 24px}
.tab.fittissima th{font-size:21px;padding:14px 24px}
...
export const CORPI_GRAFICA = {
  ...
  tabella: d => {
    ...
    return `<table class="tab ${d.righe.length >= 8 ? 'fitta fittissima' : d.righe.length >= 5 ? 'fitta' : ''}">${larg}
  ...
  // il valore si scrive con la virgola decimale, e `lab` lo sostituisce quando
  // il dato vero e' una soglia («oltre 100%») e non un numero esatto.
      ...
      ${d.barre.map((b, i) => {
        ...
        return `<g class="gx">
          ...
          <text class="val" x="${LB + w + 28}" y="${y + H / 2 + 2}" dominant-baseline="middle"
            fill="${col}">${b.lab ?? String(b.v).replace('.', ',') + unita(b.v, d.unita, d.unita1)}</text>
  ...
  impila: d => {
    ...
    return `<svg class="fig gfx" viewBox="0 0 ${LARG} ${Y + H + 168}">
      ${d.segmenti.map((s, i) => {
        ...
        return `<g class="gx">
          ...
          <text x="${num(xi + w / 2)}" y="${Y + H / 2 + 4}" text-anchor="middle"
            ...
            fill="${s.chiaro ? 'var(--tit)' : '#FFFFFF'}">${virg(s.v)}${unita(s.v, d.unita, d.unita1)}</text>
      ...
      <text x="${LARG}" y="32" class="cap" text-anchor="end"
        >${virg(tot)}${unita(tot, d.unita, d.unita1)} in tutto</text>
    </svg>`;
  },
  ...
  // Testi in <text> SVG: piano(), mai acc() — un <b> dentro <text> esce come
  // testo spezzato fuori posto (corretto nella 1.1 del corso PV).
  piramide: d => {
      ...
      ${d.strati.map((s, i) => {
        ...
        return `<g class="gx">
          ...
          <text class="lbl" x="${X + W + 60}" y="${y + H / 2 - 6}"
            dominant-baseline="middle">${piano(s.t)}</text>
          ${s.d ? `<text class="sub" x="${X + W + 60}" y="${y + H / 2 + 30}"
             dominant-baseline="middle">${piano(s.d)}</text>` : ''}
        </g>`;
      }).join('')}
    </svg>`;
  },
